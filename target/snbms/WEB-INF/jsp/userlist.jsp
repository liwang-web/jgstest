        ...
        <div class="right">
           		...
           		<form method="post" action="${pageContext.request.contextPath }/user/userList">
					<input name="method" value="query" class="input-text" type="hidden">
					 ...
					 <select name="userRole">
						   <option value="0" <c:if test="${0 == userRole }">selected="selected"</c:if>>--请选择--</option>
						   <option value="1" <c:if test="${1 == userRole }">selected="selected"</c:if>>系统管理员</option>
						   <option value="2" <c:if test="${2 == userRole }">selected="selected"</c:if>>经理</option>
						   <option value="3" <c:if test="${3 == userRole }">selected="selected"</c:if>>普通员工</option>
	        		</select>
			...
			<%--<input type="hidden" id="totalPageCount" value="${totalPageCount}"/>--%>
		  	<c:import url="rollpage.jsp">
	          	<c:param name="totalCount" value="${pages.getTotal()}"/>
	          	<c:param name="currentPageNo" value="${pages.getPageNum()}"/>
	          	<c:param name="totalPageCount" value="${pages.getPages()}"/>
          	</c:import>
        </div>
    </section>
